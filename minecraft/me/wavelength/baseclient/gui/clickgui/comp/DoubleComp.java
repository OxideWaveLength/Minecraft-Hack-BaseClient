...
public class DoubleComp extends Comp {
    ...
    public DoubleComp(Value value, Module m ,int width, int height, int x, int y) {
        this.width = width;
        this.height = height;
        this.value = value;
        this.m = m;
    ...
    public void onTyping(char typedChar, int keycode) {
        ...
        if(isNumber(typedChar) || keycode == 14 || keycode == Keyboard.KEY_ESCAPE || keycode == 52) {
    ...
    public boolean isNumber(char str) {
        boolean r = false;
        try {
            Double doub = Double.parseDouble(String.valueOf(str));
            r = true;
